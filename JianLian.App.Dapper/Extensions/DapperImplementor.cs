...
namespace DapperExtensions
{
    public interface IDapperImplementor
    {
        ...
        IEnumerable<T> GetPage<T>(IDbConnection connection, object predicate, int page, int resultsPerPage, IDbTransaction transaction, int? commandTimeout, bool buffered) where T : class;
        ...
        T GetByPredicates<T>(IDbConnection connection, object predicate, IDbTransaction transaction, int? commandTimeout) where T : class;
    }
    ...
    internal class DapperImplementor : IDapperImplementor
    {
        ...
        public T Get<T>(IDbConnection connection, dynamic id, IDbTransaction transaction, int? commandTimeout) where T : class
        {
            ...
            //var result1 = GetList<T>(connection, classMap, predicate, null, transaction, commandTimeout, true).ToList();
            T result = GetList<T>(connection, classMap, predicate, null, transaction, commandTimeout, true).SingleOrDefault();
            return result;
        }

        public T GetByPredicates<T>(IDbConnection connection, object predicate, IDbTransaction transaction, int? commandTimeout) where T : class
        {
            IClassMapper classMap = SqlGenerator.Configuration.GetMap<T>();
            IPredicate wherePredicate = GetPredicate(classMap, predicate);
            Dictionary<string, object> parameters = new Dictionary<string, object>();
            string sql = SqlGenerator.Select(classMap, wherePredicate, null, parameters);
            ...
            return connection.Query<T>(sql, dynamicParameters, transaction, false, commandTimeout, CommandType.Text).SingleOrDefault();
        }
        ...
        public void Insert<T>(IDbConnection connection, IEnumerable<T> entities, IDbTransaction transaction, int? commandTimeout) where T : class
        {
            ...
            foreach (var e in entities)
            {
                foreach (var column in properties)
                {
                    ...
                    {
                        //没有赋值key added by plx on 20180728
                        if (column.PropertyInfo.GetValue(e) == (object)Guid.Empty)
                        {
        ...
        public dynamic Insert<T>(IDbConnection connection, T entity, IDbTransaction transaction, int? commandTimeout) where T : class
        {
            ...
            foreach (var column in nonIdentityKeyProperties)
            {
                ...
                {
                    //没有赋值key
                    if (column.PropertyInfo.GetValue(entity) == (object)Guid.Empty)
                    {
                        Guid comb = SqlGenerator.Configuration.GetNextGuid();
        ...
        public IEnumerable<T> GetPage<T>(IDbConnection connection, object predicate, int page, int resultsPerPage, IDbTransaction transaction, int? commandTimeout, bool buffered) where T : class
        {
            IClassMapper classMap = SqlGenerator.Configuration.GetMap<T>();
            IPredicate wherePredicate = GetPredicate(classMap, predicate);
            return GetPage<T>(connection, classMap, wherePredicate, page, resultsPerPage, transaction, commandTimeout, buffered);
        }
        ...
        protected IEnumerable<T> GetPage<T>(IDbConnection connection, IClassMapper classMap, IPredicate predicate, int page, int resultsPerPage, IDbTransaction transaction, int? commandTimeout, bool buffered) where T : class
        {
            Dictionary<string, object> parameters = new Dictionary<string, object>();
            string sql = SqlGenerator.SelectPaged(classMap, predicate, page, resultsPerPage, parameters);
            DynamicParameters dynamicParameters = new DynamicParameters();
            foreach (var parameter in parameters)
            {
                dynamicParameters.Add(parameter.Key, parameter.Value);
