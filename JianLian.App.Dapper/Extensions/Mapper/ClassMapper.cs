    ...
    public class ClassMapper<T> : IClassMapper<T> where T : class
    {
            ...
            PropertyTypeKeyTypeMapping = new Dictionary<Type, KeyType>
                                             {
                                                 ...
                                                 //{ typeof(Guid), KeyType.Identity }, { typeof(Guid?), KeyType.Identity },
        ...
        protected virtual void AutoMap(Func<Type, PropertyInfo, bool> canMap)
        {
            ...
            foreach (var propertyInfo in type.GetProperties())
            {
                ...
                if (!hasDefinedKey)
                {
                    if (string.Equals(map.PropertyInfo.Name, "Key", StringComparison.InvariantCultureIgnoreCase))
                    {
                        keyMap = map;
                    }

                    if (keyMap == null && map.PropertyInfo.Name.EndsWith("Key", true, CultureInfo.InvariantCulture))
                    {
                        keyMap = map;
                    }
                }
