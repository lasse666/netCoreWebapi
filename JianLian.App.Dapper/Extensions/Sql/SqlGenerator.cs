...
namespace DapperExtensions.Sql
{
    public interface ISqlGenerator
    {
        ...
        string SelectPaged(IClassMapper classMap, IPredicate predicate, int page, int resultsPerPage, IDictionary<string, object> parameters);
    ...
    public class SqlGeneratorImpl : ISqlGenerator
    {
        ...
        /// <returns></returns>
        public virtual string SelectPaged(IClassMapper classMap, IPredicate predicate,  int page, int resultsPerPage, IDictionary<string, object> parameters)
        {
            if (parameters == null)
            {
                throw new ArgumentNullException("Parameters");
        ...
        {
            var columns = classMap.Properties.Where(p => !(p.Ignored || p.IsReadOnly));// || p.KeyType == KeyType.Identity)); edited by plx
            if (!columns.Any())
            {
        ...
        public virtual string Update(IClassMapper classMap, IPredicate predicate, IDictionary<string, object> parameters)
        {
            ...
            var columns = classMap.Properties.Where(p => !(p.Ignored || p.IsReadOnly || p.KeyType == KeyType.Identity) && p.PropertyInfo.PropertyType != typeof(byte[]));//edited by plx
            if (!columns.Any())
            {
                throw new ArgumentException("No columns were mapped.");
            }
